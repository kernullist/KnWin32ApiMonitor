...
`generated/tier1-hook-plan.json` currently plans all 15,932 Tier 1 APIs. The plan marks 15,528 APIs as generic pointer/scalar fallback ready, 404 APIs as blocked by the current generic ABI slot policy, and 1,795 APIs as requiring explicit allowlist before installation because of risk policy. The default profile installs zero Tier 1 hooks; broad coverage requires an explicit profile, module/family/risk selector, or allowlist.
...
`generated/dll-batch-promotion-plan.json` currently groups 179 committed definitions across 25 DLLs. It records 129 already-live APIs, zero auto-promotable pointer/scalar APIs, 1 manual-decoder-required API, 49 payload-policy-blocked APIs, and zero unsupported committed definitions. `generated/manual-decoder-batch-plan.json` currently groups the remaining manual-decoder-required API, `rpcrt4.dll!RpcMgmtEpEltInqDone`, tracks 3 payload-blocked sibling APIs in that DLL, and keeps the manual candidate blocked by default. The auto-promotable DLL queue is currently empty; the next runtime promotion should start from this remaining manual decoder batch gate or explicitly reviewed payload-sensitive families instead of unrelated single-API work.
...
1. `definition_only`: 50
2. `hooked`: 4
3. `smoke_verified`: 125

The current Microsoft-source inventory report totals 18,086 monitorable catalog rows:

1. `tier 0`: 129 currently emitted and decoded agent-hook APIs.
2. `tier 1`: 15,932 DLL-export APIs with parameter metadata that need hook/decoder implementation.
