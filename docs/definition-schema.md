...
4. Wave 2 API IDs `14` through `90`, with selected registry, bcrypt CNG provider/RNG, crypt32 certificate-store/message-handle, RPCRT4 binding, Winsock, and WinHTTP session IDs promoted from definition-only to smoke-verified IAT coverage.
5. Module IDs:
   - `kernel32.dll = 1`
   - `ntdll.dll = 2`
   - `kernelbase.dll = 3`
...
The live Wave 2 slices are limited to smoke-verified `advapi32.dll` registry, `bcrypt.dll` CNG provider/RNG, `crypt32.dll` certificate-store/message-handle, `rpcrt4.dll` local RPC binding, `ws2_32.dll` bootstrap/address-resolution, and `winhttp.dll` session-handle IAT hooks. The selected registry APIs are `RegOpenKeyExW`, `RegCreateKeyExW`, `RegQueryValueExW`, `RegSetValueExW`, `RegDeleteValueW`, and `RegCloseKey`. The selected bcrypt APIs are `BCryptOpenAlgorithmProvider`, `BCryptCloseAlgorithmProvider`, `BCryptGetProperty`, and `BCryptGenRandom`. The selected crypt32 APIs are `CertOpenStore`, `CertCloseStore`, `CryptMsgOpenToDecode`, and `CryptMsgClose`. The selected RPCRT4 APIs are `RpcStringBindingComposeW`, `RpcBindingFromStringBindingW`, `RpcStringFreeW`, and `RpcBindingFree`. The selected Winsock APIs are `WSAStartup`, `WSACleanup`, `socket`, `closesocket`, `getaddrinfo`, `freeaddrinfo`, and `WSAGetLastError`. The selected WinHTTP APIs are `WinHttpOpen` and `WinHttpCloseHandle`. All other Wave 2 APIs remain `definition_only`.
