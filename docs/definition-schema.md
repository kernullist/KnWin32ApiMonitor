...
21. Wave 4 API IDs `157` through `179`, covering OLE Automation, SSPI, user profile/environment, DNS, IP Helper, SetupAPI, Shell lightweight path helpers, WinTrust, and DbgHelp lifecycle/metadata entrypoints; `oleaut32.dll` IDs `158` through `160`, `userenv.dll` ID `167`, `dnsapi.dll` ID `169`, `iphlpapi.dll` ID `171`, `setupapi.dll` ID `173`, plus `dbghelp.dll` IDs `178` and `179` are low-payload Wave 4 smoke-verified hooks.
22. Module IDs:
   - `kernel32.dll = 1`
   - `ntdll.dll = 2`
   - `kernelbase.dll = 3`
...
`generated/tier1-hook-plan.json` currently plans all 15,938 Tier 1 APIs. The plan marks 15,534 APIs as generic pointer/scalar fallback ready, 404 APIs as blocked by the current generic ABI slot policy, and 1,795 APIs as requiring explicit allowlist before installation because of risk policy. The default profile installs zero Tier 1 hooks; broad coverage requires an explicit profile, module/family/risk selector, or allowlist.
...
2. `hooked`: 4
3. `smoke_verified`: 119

The current Microsoft-source inventory report totals 18,086 monitorable catalog rows:

1. `tier 0`: 123 currently emitted and decoded agent-hook APIs.
2. `tier 1`: 15,938 DLL-export APIs with parameter metadata that need hook/decoder implementation.
