...
`generated/tier2-profile-batch-plan.json` is generated from the Tier 2 hook plan. It groups resolved API-set rows by concrete host DLL, groups missing-parameter return-only rows by source DLL, keeps unresolved API-set rows in explicit blocked batches, and records the review evidence required before any broader Tier 2 profile can become runtime-installable. It is a planning and validation artifact only: runtime hooks remain disabled by default.
