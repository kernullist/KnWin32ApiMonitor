...
21. Wave 4 API IDs `157` through `179`, covering OLE Automation, SSPI, user profile/environment, DNS, IP Helper, SetupAPI, Shell lightweight path helpers, WinTrust, and DbgHelp lifecycle/metadata entrypoints; `dbghelp.dll` IDs `178` and `179` are the first low-payload Wave 4 smoke-verified hooks.
22. Module IDs:
   - `kernel32.dll = 1`
   - `ntdll.dll = 2`
   - `kernelbase.dll = 3`
...
1. `definition_only`: 63
2. `hooked`: 4
3. `smoke_verified`: 112

The unselected Wave 4 common-DLL definitions remain in the `definition_only` count. The selected DbgHelp symbol-session pair is counted as `smoke_verified` and adds only pointer/scalar target hook evidence.
