...
4. Wave 2 API IDs `14` through `90`, with selected registry, bcrypt CNG provider/RNG, crypt32 certificate-store/message-handle, RPCRT4 binding, Winsock, WinHTTP session, and WinINet session IDs promoted from definition-only to smoke-verified IAT coverage.
5. Module IDs:
   - `kernel32.dll = 1`
   - `ntdll.dll = 2`
   - `kernelbase.dll = 3`
