...
21. Wave 4 API IDs `157` through `179`, covering OLE Automation, SSPI, user profile/environment, DNS, IP Helper, SetupAPI, Shell lightweight path helpers, WinTrust, and DbgHelp lifecycle/metadata entrypoints; `oleaut32.dll` IDs `159` and `160`, `userenv.dll` ID `167`, `dnsapi.dll` ID `169`, `iphlpapi.dll` ID `171`, `setupapi.dll` ID `173`, plus `dbghelp.dll` IDs `178` and `179` are low-payload Wave 4 smoke-verified hooks.
22. Module IDs:
   - `kernel32.dll = 1`
   - `ntdll.dll = 2`
   - `kernelbase.dll = 3`
