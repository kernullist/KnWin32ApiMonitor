...
`generated/tier1-hook-plan.json` currently plans all 15,934 Tier 1 APIs. The plan marks 15,530 APIs as generic pointer/scalar fallback ready, 404 APIs as blocked by the current generic ABI slot policy, and 1,795 APIs as requiring explicit allowlist before installation because of risk policy. The default profile installs zero Tier 1 hooks; broad coverage requires an explicit profile, module/family/risk selector, or allowlist.
...
5. `iphlpapi.dll`: adapter/interface metadata query definitions, with `GetIfEntry2` promoted to low-payload pointer-only smoke-verified coverage.
...
2. `hooked`: 4
3. `smoke_verified`: 123

The current Microsoft-source inventory report totals 18,086 monitorable catalog rows:

1. `tier 0`: 127 currently emitted and decoded agent-hook APIs.
2. `tier 1`: 15,934 DLL-export APIs with parameter metadata that need hook/decoder implementation.
