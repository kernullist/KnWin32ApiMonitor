...
`generated/definition-decoder-tables.json` and `GeneratedApiMetadata.h` include API rows, parameter rows, decode alias rows, and enum/flag value-set rows. Parameters inherit alias-level `enum` and `flags` bindings unless they override them directly, so newly added APIs can get analyst-facing constant names by referencing shared decode aliases or metadata sets instead of adding per-API UI code.
...
generated/tier2-profile-review-manifest.json
...
`generated/tier2-profile-batch-plan.json` is generated from the Tier 2 hook plan. It groups resolved API-set rows by concrete host DLL, groups missing-parameter return-only rows by source DLL, keeps unresolved API-set rows in explicit blocked batches, and records the review evidence required before any broader Tier 2 profile can become runtime-installable. `defs:tier2-profile-batch-plan:select` previews those batches by batch id, kind, profile, resolved host, source module, family, or risk. It is a planning and validation artifact only: runtime hooks remain disabled by default.

`generated/tier2-profile-review-manifest.json` is generated from the Tier 2 profile batch plan. It ranks every Tier 2 batch into `initial_candidate`, `review_ready`, `explicit_allowlist_required`, or `blocked` review classes. Initial candidates are small no-allowlist batches only; every row still requires target provider evidence, x64 and x86 native smoke, zero healthy-path drops, and hook-overhead review before runtime promotion. It is a planning and validation artifact only: runtime hooks remain disabled by default.
...
10. Generated controller-side decoder table and enum/flag value-set freshness checks.
...
17. Tier 2 profile batch-plan validation, including exact Tier 2 coverage across resolved-host, return-only, and blocked batches, default-disabled runtime policy checks, high-risk allowlist accounting, and deterministic JSON ordering.
18. Tier 2 profile review-manifest validation, including exact Tier 2 batch coverage, initial-candidate policy checks, allowlist-gated and blocked batch accounting, default-disabled runtime policy checks, and deterministic JSON ordering.
19. Tier 3 hook-plan validation, including 100% Tier 3 plan coverage, callback/COM/message/buffer/security classification checks, default-disabled policy checks, and deterministic JSON ordering.
20. DLL batch promotion plan validation, including 100% committed-definition coverage, same-DLL grouping, auto/manual/blocked classification checks, high-risk auto-promotion rejection, and deterministic JSON ordering.
21. Manual decoder batch plan validation, including exact coverage of `manual_decoder_required` APIs from the DLL batch plan, blocked-by-default runtime policy checks, allowed/forbidden evidence boundary checks, same-DLL payload-blocked sibling tracking, and deterministic JSON ordering.
...
Enum values live in `definitions/metadata/enums.json`; flag values live in `definitions/metadata/flags.json`. Numeric values may be decimal or hex strings and are normalized by tooling for validation/reporting. The generated controller metadata exports these sets as constant tables, and the controller appends matching enum names or decomposed flag names to decoded numeric argument values while keeping the injected agent path compact.
...
The live Wave 2/3/4 slices are limited to smoke-verified `advapi32.dll` registry, selected `advapi32.dll` token query/privilege lookup, `bcrypt.dll` CNG provider/RNG/key-destroy, `crypt32.dll` certificate-store/message-handle, `rpcrt4.dll` local RPC binding, binding option, endpoint inquiry cleanup, and UUID helpers, `ws2_32.dll` bootstrap/address-resolution/connect metadata, `winhttp.dll` session/scalar-option metadata, `wininet.dll` session-handle, selected COMBASE-backed WinRT lifecycle, selected KERNEL32 memory protection, selected KERNEL32 thread lifecycle, selected KERNEL32 event synchronization, selected KERNEL32 mutex/semaphore synchronization, selected KERNEL32 file-mapping, selected KERNEL32 process/thread identity, selected KERNEL32 handle metadata, selected KERNEL32 module lifecycle, selected KERNEL32 file metadata, selected `oleaut32.dll` lifecycle IAT hooks, selected `secur32.dll` credential/context cleanup IAT hooks, selected `userenv.dll` environment-block destroy IAT hook, selected `dnsapi.dll` record-list free IAT hook, selected `iphlpapi.dll` adapter/interface metadata IAT hooks, selected `setupapi.dll` device-info close IAT hook, selected `shlwapi.dll` path-exists query IAT hook, selected `wintrust.dll` state-query IAT hook, and selected `dbghelp.dll` symbol-session IAT hooks. The selected registry APIs are `RegOpenKeyExW`, `RegCreateKeyExW`, `RegQueryValueExW`, `RegSetValueExW`, `RegDeleteValueW`, and `RegCloseKey`. The selected token query APIs are `OpenProcessToken` and `LookupPrivilegeValueW`. The selected bcrypt APIs are `BCryptOpenAlgorithmProvider`, `BCryptCloseAlgorithmProvider`, `BCryptGetProperty`, `BCryptGenRandom`, and `BCryptDestroyKey`. The selected crypt32 APIs are `CertOpenStore`, `CertCloseStore`, `CryptMsgOpenToDecode`, and `CryptMsgClose`. The selected RPCRT4 APIs are `RpcStringBindingComposeW`, `RpcBindingFromStringBindingW`, `RpcStringFreeW`, `RpcBindingFree`, `RpcBindingSetOption`, `RpcMgmtEpEltInqDone`, `UuidCreate`, `UuidToStringW`, and `UuidFromStringW`. The selected Winsock APIs are `WSAStartup`, `WSACleanup`, `socket`, `closesocket`, `connect`, `getaddrinfo`, `freeaddrinfo`, and `WSAGetLastError`. The selected WinHTTP APIs are `WinHttpOpen`, `WinHttpCloseHandle`, and `WinHttpSetOption`. The selected WinINet APIs are `InternetOpenW` and `InternetCloseHandle`. The selected COMBASE-backed WinRT lifecycle APIs are `RoInitialize`, `RoUninitialize`, and `RoGetApartmentIdentifier` from the observed `api-ms-win-core-winrt-l1-1-0.dll` import provider. The selected KERNEL32 memory APIs are `VirtualAlloc`, `VirtualFree`, `VirtualProtect`, and `VirtualQuery`. The selected KERNEL32 thread APIs are `CreateThread`, `OpenThread`, `WaitForSingleObject`, and `GetExitCodeThread`. The selected KERNEL32 event synchronization APIs are `CreateEventW`, `OpenEventW`, `SetEvent`, `ResetEvent`, and `WaitForSingleObjectEx`. The selected KERNEL32 mutex/semaphore synchronization APIs are `CreateMutexW`, `OpenMutexW`, `ReleaseMutex`, `CreateSemaphoreW`, `OpenSemaphoreW`, `ReleaseSemaphore`, and `WaitForMultipleObjectsEx`. The selected KERNEL32 file-mapping APIs are `CreateFileMappingW`, `OpenFileMappingW`, `MapViewOfFile`, and `UnmapViewOfFile`. The selected KERNEL32 process/thread identity APIs are `GetCurrentProcess`, `GetCurrentProcessId`, `GetCurrentThread`, `GetCurrentThreadId`, `GetProcessId`, and `GetThreadId`. The selected KERNEL32 handle metadata APIs are `GetStdHandle`, `GetFileType`, `GetHandleInformation`, and `SetHandleInformation`. The selected KERNEL32 module lifecycle APIs are `GetModuleHandleW`, `GetModuleHandleExW`, `GetModuleFileNameW`, and `FreeLibrary`. The selected KERNEL32 file metadata APIs are `GetFileSizeEx`, `GetFileTime`, and `GetFileInformationByHandle`. The selected OLEAUT32 APIs are `SysFreeString`, `VariantClear`, and `SafeArrayDestroy`. The selected SECUR32 APIs are `FreeCredentialsHandle` and `DeleteSecurityContext`. The selected USERENV API is `DestroyEnvironmentBlock`. The selected DNSAPI API is `DnsRecordListFree`, normalized from the modern `DnsFree` import provider for `DnsFreeRecordList` calls. The selected IPHLPAPI APIs are `GetAdaptersAddresses` and `GetIfEntry2`. The selected SETUPAPI API is `SetupDiDestroyDeviceInfoList`. The selected SHLWAPI API is `PathFileExistsW`. The selected WINTRUST API is `WTHelperProvDataFromStateData`. The selected DbgHelp APIs are `SymInitializeW` and `SymCleanup`. `connect` keeps stable API ID `63` and records only socket handle, sockaddr pointer/length, decoded loopback endpoint metadata, return/error/timing, and hook lifecycle evidence. `RpcBindingSetOption` keeps stable API ID `54` and records only binding handle, option ID, scalar option value, return/timing, and hook lifecycle evidence. `RpcMgmtEpEltInqDone` keeps stable API ID `57` and records only the `RPC_EP_INQ_HANDLE*` pointer address, pre-call inquiry handle value, post-call inquiry handle value, `RPC_STATUS`, timing, and hook lifecycle evidence without calling `RpcMgmtEpEltInqNextW`, enumerating endpoint mapper entries, copying annotations, expanding RPC interface or object UUID data, copying binding vectors/string bindings, capturing auth data, or emitting byte-preview payload capture. `WinHttpSetOption` keeps stable API ID `89` and records only WinHTTP handle, option ID, option-buffer pointer, option-buffer length, allowlisted DWORD scalar value, return/error/timing, and hook lifecycle evidence without request, transfer, header, body, cookie, credential, proxy credential, or raw option-buffer payload capture. `BCryptDestroyKey` keeps stable API ID `34` and records only the `BCRYPT_KEY_HANDLE` value plus `NTSTATUS` return/error/timing without copying key material or crypto buffers. `SysFreeString` keeps stable API ID `158` and records only the BSTR pointer, void return/timing, and hook lifecycle evidence without dereferencing or copying BSTR text, user paths, credentials, or byte-preview payload capture. `VariantClear` keeps stable API ID `159` and records only the `VARIANTARG*` pointer, HRESULT return/error/timing, and hook lifecycle evidence without BSTR text, VARIANT contents, user paths, credentials, or byte-preview payload capture. `SafeArrayDestroy` keeps stable API ID `160` and records only the `SAFEARRAY*` pointer, HRESULT return/error/timing, and hook lifecycle evidence without SAFEARRAY bounds, array data, array elements, user paths, credentials, or byte-preview payload capture. `FreeCredentialsHandle` keeps stable API ID `162` and records only the `PCredHandle` pointer, `SECURITY_STATUS` return/error/timing, and hook lifecycle evidence without copying principals, package names, auth data, logon IDs, SSPI security buffers, context tokens, target names, passwords, secrets, or byte-preview payload capture. `DeleteSecurityContext` keeps stable API ID `164` and records only the `PCtxtHandle` pointer, `SECURITY_STATUS` return/error/timing, and hook lifecycle evidence without copying SSPI security buffers, context tokens, target names, credentials, or byte-preview payload capture. `DestroyEnvironmentBlock` keeps stable API ID `167` and records only the environment-block pointer, BOOL/error/timing, and hook lifecycle evidence without copying environment variables, profile directories, user paths, command lines, credentials, or byte-preview payload capture. `DnsRecordListFree` keeps stable API ID `169` and records only the DNS record-list pointer, `FreeType` scalar, timing, and hook lifecycle evidence without calling `DnsQuery_W`, copying DNS names, decoding DNS records, inspecting adapter or route inventory, or emitting byte-preview payload capture. `GetAdaptersAddresses` keeps stable API ID `170` and records only family, flags, reserved pointer, adapter-address buffer pointer, `SizePointer` pre/post size, `ULONG` return/error/timing, and hook lifecycle evidence without copying adapter addresses, interface names, descriptions, MAC or physical addresses, IP addresses, DNS suffixes, gateways, routes, traffic counters, or byte-preview payload capture. `GetIfEntry2` keeps stable API ID `171` and records only the `PMIB_IF_ROW2` pointer, `NETIO_STATUS` return/error/timing, and hook lifecycle evidence without copying adapter addresses, interface names, descriptions, MAC or physical addresses, IP addresses, DNS suffixes, gateways, routes, traffic counters, or byte-preview payload capture. `SetupDiDestroyDeviceInfoList` keeps stable API ID `173` and records only the `HDEVINFO` handle, BOOL/error/timing, and hook lifecycle evidence without calling `SetupDiGetClassDevsW`, enumerating devices, copying device instance IDs, hardware IDs, compatible IDs, class GUIDs, paths, or emitting byte-preview payload capture. `PathFileExistsW` keeps stable API ID `174` and records only the `LPCWSTR` path pointer, BOOL return/error/timing, and hook lifecycle evidence without copying path text, combining/canonicalizing/parsing shell paths, calling `ShellExecute`, capturing PIDLs, command lines, environment variables, credentials, or byte-preview payload capture. `WTHelperProvDataFromStateData` keeps stable API ID `177` and records only the input state handle, returned `CRYPT_PROVIDER_DATA` pointer value, timing, and hook lifecycle evidence without calling `WinVerifyTrust`, dereferencing provider data, walking signer/certificate/catalog state, copying paths, inspecting hashes, capturing credentials, or emitting byte-preview payload capture. `SymInitializeW` keeps stable API ID `178` and records only process handle, pointer-only search-path evidence, BOOL invade flag, return/error/timing, and hook lifecycle evidence without symbol path, PDB, module-list, stack, or byte-preview payload capture. All other unselected Wave 2 APIs and unselected Wave 4 APIs remain `definition_only`.

Wave 4 adds 23 common-DLL APIs across:

1. `oleaut32.dll`: BSTR, VARIANT, and SAFEARRAY lifecycle definitions, with `SysFreeString`, `VariantClear`, and `SafeArrayDestroy` promoted to low-payload smoke-verified hooks.
2. `secur32.dll`: SSPI credential and context lifecycle definitions, with `FreeCredentialsHandle` and `DeleteSecurityContext` promoted to low-payload credential/context cleanup smoke-verified coverage.
3. `userenv.dll`: profile-directory and environment-block definitions, with `DestroyEnvironmentBlock` promoted to low-payload smoke-verified destroy-only coverage.
4. `dnsapi.dll`: DNS query and record-list lifecycle definitions, with `DnsRecordListFree` promoted to low-payload smoke-verified coverage through the `DnsFree` import provider.
5. `iphlpapi.dll`: adapter/interface metadata query definitions, with `GetAdaptersAddresses` and `GetIfEntry2` promoted to low-payload smoke-verified coverage.
6. `setupapi.dll`: device-info set lifecycle definitions, with `SetupDiDestroyDeviceInfoList` promoted to low-payload smoke-verified close-only coverage.
7. `shlwapi.dll`: shell path helper definitions, with `PathFileExistsW` promoted to low-payload pointer-only smoke-verified coverage.
8. `wintrust.dll`: trust verification/state query definitions, with `WTHelperProvDataFromStateData` promoted to low-payload pointer-only smoke-verified coverage.
9. `dbghelp.dll`: symbol-session lifecycle definitions, with `SymInitializeW` and `SymCleanup` promoted to low-payload smoke-verified hooks.

Wave 4 remains metadata-only except for the selected OLEAUT32 BSTR/variant/safe-array lifecycle hooks, selected SECUR32 credential/context cleanup hooks, selected USERENV environment-block destroy hook, selected DNSAPI record-list free hook, selected IPHLPAPI adapter/interface metadata hooks, selected SETUPAPI device-info close hook, selected SHLWAPI path-exists query hook, selected WINTRUST state-query hook, and selected DbgHelp symbol-session lifecycle pair. The unselected Wave 4 APIs have stable IDs and generated controller metadata, but they are not live hooks and do not add target-process overhead until each family gets a separate hook ABI, payload-safety, and performance review.
...
`defs:decoder-tables` verifies the generated decoder metadata artifact covers API IDs `1` through `179`, parameter rows, decode alias rows, enum/flag value-set rows, length-source resolution, and valid parameter references to shared enum/flag sets.

`defs:inventory` regenerates the Microsoft-source inventory and its coverage report. `defs:tier2-profile-batch-plan:generate` regenerates the Tier 2 resolved-host/source-DLL review batches from the Tier 2 hook plan, `defs:tier2-profile-batch-plan:select` previews candidate review batches without enabling runtime hooks, and `defs:tier2-profile-review:generate` regenerates the ranked Tier 2 batch review manifest. `defs:dll-batch-plan:generate` regenerates the committed-definition DLL batch promotion queue. `defs:manual-decoder-plan:generate` regenerates the manual-decoder batch gate from the DLL batch queue. `defs:coverage` prints a deterministic Markdown report grouped by module, family, risk, hook policy, coverage status, and decode quality, then appends Microsoft-source inventory totals, hook tiers, next API families by coverage impact, DLL batch auto/manual/blocked promotion buckets, and manual-decoder batch gates. The definition report explicitly separates:
