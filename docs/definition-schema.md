...
`generated/definition-decoder-tables.json` is the deterministic JSON view for tooling. It contains module rows, API rows through ID `145`, flattened parameter rows, decode alias rows, and source file lists without wall-clock timestamps or local machine paths.
...
The current definition coverage report totals 145 APIs:
