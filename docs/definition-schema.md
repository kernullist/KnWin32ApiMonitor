...
1. `definition_only`: 59
2. `hooked`: 4
3. `smoke_verified`: 116

The unselected Wave 4 common-DLL definitions remain in the `definition_only` count. The selected OLEAUT32 lifecycle pair, DNSAPI record-list free hook, SETUPAPI device-info close hook, and DbgHelp symbol-session pair are counted as `smoke_verified` and add only pointer/scalar target hook evidence.
