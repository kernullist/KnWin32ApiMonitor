...
`generated/tier1-hook-plan.json` currently plans all 15,937 Tier 1 APIs. The plan marks 15,533 APIs as generic pointer/scalar fallback ready, 404 APIs as blocked by the current generic ABI slot policy, and 1,795 APIs as requiring explicit allowlist before installation because of risk policy. The default profile installs zero Tier 1 hooks; broad coverage requires an explicit profile, module/family/risk selector, or allowlist.
...
1. `definition_only`: 55
2. `hooked`: 4
3. `smoke_verified`: 120

The current Microsoft-source inventory report totals 18,086 monitorable catalog rows:

1. `tier 0`: 124 currently emitted and decoded agent-hook APIs.
2. `tier 1`: 15,937 DLL-export APIs with parameter metadata that need hook/decoder implementation.
