...
4. Wave 2 API IDs `14` through `90`, with selected registry, token query/privilege lookup, bcrypt CNG provider/RNG, crypt32 certificate-store/message-handle, RPCRT4 binding and `UuidCreate` ID `58`, Winsock, WinHTTP session, and WinINet session IDs promoted from definition-only to smoke-verified IAT coverage.
...
`generated/definition-decoder-tables.json` is the deterministic JSON view for tooling. It contains module rows, API rows through ID `153`, flattened parameter rows, decode alias rows, and source file lists without wall-clock timestamps or local machine paths.
...
The current definition coverage report totals 153 APIs:

1. `definition_only`: 44
2. `hooked`: 4
3. `smoke_verified`: 105
...
`defs:decoder-tables` verifies the generated decoder metadata artifact covers API IDs `1` through `145`, parameter rows, decode alias rows, and length-source resolution.
