...
The IPHLPAPI interface-entry query slice records only `GetIfEntry2` `PMIB_IF_ROW2` pointer, `NETIO_STATUS` return/error evidence, timing, and hook lifecycle metadata. The sample uses `GetIfTable2` only to obtain a valid input row and immediately releases it with `FreeMibTable`; it does not copy adapter addresses, interface names, descriptions, MAC or physical addresses, IP addresses, DNS suffixes, gateways, routes, traffic counters, or arbitrary byte previews.
