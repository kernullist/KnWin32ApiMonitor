...
The OLEAUT32 lifecycle slice records only `VariantClear` and `SafeArrayDestroy` pointer values, HRESULT return evidence, timing, and hook lifecycle metadata. It does not copy BSTR text, VARIANT contents, SAFEARRAY bounds/data, array elements, user paths, credentials, or arbitrary byte previews.

The DNSAPI record-list free slice records only `DnsRecordListFree` pointer and `FreeType` scalar evidence, timing, and hook lifecycle metadata. Modern Windows SDK headers lower `DnsRecordListFree` to the `DnsFree` import provider, so the agent patches `DnsFree` but emits stable `DnsRecordListFree` API ID `169` only for `DnsFreeRecordList` calls. It does not call `DnsQuery_W`, copy DNS names, decode DNS records, inspect adapter or route inventory, or emit arbitrary byte previews.

The SETUPAPI device-info close slice records only `SetupDiDestroyDeviceInfoList` `HDEVINFO` handle, BOOL/error evidence, timing, and hook lifecycle metadata. The sample creates an empty device-info set with `SetupDiCreateDeviceInfoList(nullptr, nullptr)` and destroys it; it does not call `SetupDiGetClassDevsW`, enumerate devices, copy device instance IDs, hardware IDs, compatible IDs, class GUIDs, paths, or emit arbitrary byte previews.

The USERENV environment-block destroy slice records only `DestroyEnvironmentBlock` environment-block pointer, BOOL/error evidence, timing, and hook lifecycle metadata. The sample creates a block through `CreateEnvironmentBlock(&environment, nullptr, FALSE)` only to obtain a valid block to destroy; it does not copy environment variables, profile directories, user paths, command lines, credentials, or arbitrary byte previews.

The IPHLPAPI adapter/interface metadata slice records only `GetAdaptersAddresses` family/flags, reserved pointer, adapter-address buffer pointer, `SizePointer` pre/post size, `ULONG` return/error evidence, plus `GetIfEntry2` `PMIB_IF_ROW2` pointer, `NETIO_STATUS` return/error evidence, timing, and hook lifecycle metadata. The sample uses `GetAdaptersAddresses` only as a bounded size/query probe and uses `GetIfTable2` only to obtain a valid `GetIfEntry2` input row before immediately releasing it with `FreeMibTable`; it does not copy adapter addresses, interface names, descriptions, MAC or physical addresses, IP addresses, DNS suffixes, gateways, routes, traffic counters, or arbitrary byte previews.

The SHLWAPI path-query slice records only `PathFileExistsW` path pointer, BOOL return/error evidence, timing, and hook lifecycle metadata. The sample calls `PathFileExistsW(L"C:\\Windows")` only as a deterministic success probe; it does not copy path text, combine or canonicalize shell paths, call `ShellExecute`, capture PIDLs, command lines, environment variables, credentials, or arbitrary byte previews.

The WinTrust state-query slice records only `WTHelperProvDataFromStateData` state handle, returned `CRYPT_PROVIDER_DATA` pointer value, timing, and hook lifecycle metadata. The sample calls `WTHelperProvDataFromStateData(nullptr)` only as a deterministic null-state query; it does not call `WinVerifyTrust`, dereference provider data, walk signer/certificate/catalog state, copy paths, inspect hashes, capture credentials, or emit arbitrary byte previews.

The DbgHelp symbol-session slice records only `SymInitializeW` and `SymCleanup` process handles, pointer-only `UserSearchPath`, the `fInvadeProcess` BOOL, return/error evidence, timing, and hook lifecycle metadata. It does not copy symbol paths, PDB names, loaded-module details, stack frames, module memory, or byte previews.
...
The RPCRT4 endpoint inquiry cleanup slice records only the `RPC_EP_INQ_HANDLE*` pointer address, pre-call inquiry handle value, post-call inquiry handle value, `RPC_STATUS`, timing, and hook lifecycle evidence for `RpcMgmtEpEltInqDone`. The sample calls `RpcMgmtEpEltInqBegin` only to obtain a handle and does not call `RpcMgmtEpEltInqNextW`; the hook does not enumerate endpoint mapper entries, copy annotations, expand `RPC_IF_ID` or object UUID fields, inspect string bindings or binding vectors, capture auth data, make RPC network calls, or emit arbitrary buffer previews.
...
`Protocol.h` includes `GeneratedApiIds.h`, so the current compact shared-memory transport API and module IDs are generated from the stable assignment metadata. The generated values preserve the existing File I/O ids `1` through `6`, loader ids `7` through `11`, Wave 2 ids `14` through `90` including smoke-verified `RpcMgmtEpEltInqDone` id `57`, Phase 13G-promoted `UuidCreate` id `58`, and WinHTTP scalar option API `WinHttpSetOption` id `89`, Phase 13B User32/GDI32 ids `91` through `97`, Phase 13C PSAPI ids `98` through `101`, Phase 13D Version ids `102` through `104`, Phase 13E Shell ids `105` through `106`, Phase 13F OLE32 ids `107` through `110`, Phase 13G RPCRT4 UUID helper ids `111` through `112`, Phase 13H COMBASE-backed WinRT lifecycle ids `113` through `115`, Phase 13I KERNEL32 memory protection ids `116` through `119`, Phase 13J KERNEL32 thread lifecycle ids `120` through `123`, Phase 13K KERNEL32 event synchronization ids `124` through `128`, Phase 13L KERNEL32 mutex/semaphore synchronization ids `129` through `135`, Phase 13M KERNEL32 file-mapping ids `136` through `139`, Phase 13N KERNEL32 process/thread identity ids `140` through `145`, Phase 13O KERNEL32 handle metadata ids `146` through `149`, Phase 13P KERNEL32 module lifecycle ids `150` through `153`, Phase 13Q KERNEL32 file metadata ids `154` through `156`, and Wave 4 common-DLL ids `157` through `179` including smoke-verified OLEAUT32 lifecycle ids `158` through `160`, SECUR32 credential/context cleanup ids `162` and `164`, USERENV environment-block destroy id `167`, DNSAPI record-list free id `169`, IPHLPAPI adapter/interface metadata ids `170` and `171`, SETUPAPI device-info close id `173`, SHLWAPI path-exists id `174`, WINTRUST state-query id `177`, plus DbgHelp symbol-session ids `178` and `179`; `psapi.dll` is module id `13`, `version.dll` is module id `14`, `shell32.dll` is module id `15`, `ole32.dll` is module id `16`, `api-ms-win-core-winrt-l1-1-0.dll` is module id `17`, `oleaut32.dll` is module id `18`, `secur32.dll` is module id `19`, `userenv.dll` is module id `20`, `dnsapi.dll` is module id `21`, `iphlpapi.dll` is module id `22`, `setupapi.dll` is module id `23`, `shlwapi.dll` is module id `24`, `wintrust.dll` is module id `25`, and `dbghelp.dll` is module id `26`.
