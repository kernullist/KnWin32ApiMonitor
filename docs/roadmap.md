...
20. `generated/manual-decoder-batch-plan.json` is generated and validated from the DLL batch queue. It now covers zero DLLs and zero APIs because the previous `rpcrt4.dll!RpcMgmtEpEltInqDone` manual candidate has been promoted to a smoke-verified low-payload cleanup hook.
...
7. DLL batch promotion plan: 25 DLLs, 130 already-live APIs, zero auto-promotable pointer/scalar APIs, zero manual-decoder-required APIs, 49 payload-policy-blocked APIs, and zero broad hooks enabled by default.
8. Manual decoder batch plan: zero DLLs, zero manual-decoder-required APIs, and zero runtime hooks enabled by default.
