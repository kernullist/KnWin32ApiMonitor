...
14. The coverage report currently totals 50 `definition_only`, 4 `hooked`, and 125 `smoke_verified` APIs.
...
20. `generated/manual-decoder-batch-plan.json` is generated and validated from the DLL batch queue. It now covers only `rpcrt4.dll!RpcMgmtEpEltInqDone`, tracks same-DLL payload-blocked siblings, records allowed/forbidden evidence boundaries, and keeps the API blocked by default until same-DLL design review, x64/x86 smoke, payload absence, and overhead gates pass.
...
Status: refreshed on 2026-06-20 after low-payload WINTRUST state-query promotion and manual-decoder batch gate regeneration.

Current definition coverage:

1. Total APIs: 179.
2. `smoke_verified`: 125.
3. `hooked`: 4.
4. `definition_only`: 50.
5. Microsoft-source inventory: 18,086 APIs, with 129 Tier 0 rows, 15,932 Tier 1 rows, 655 Tier 2 rows, and 1,370 Tier 3 rows.
...
7. DLL batch promotion plan: 25 DLLs, 129 already-live APIs, zero auto-promotable pointer/scalar APIs, 1 manual-decoder-required API, 49 payload-policy-blocked APIs, and zero broad hooks enabled by default.
8. Manual decoder batch plan: 1 DLL, 1 manual-decoder-required API, 3 same-DLL payload-blocked sibling APIs, and zero runtime hooks enabled by default.
...
6. Use the DLL batch promotion plan as the default promotion queue:
   ...
   - The auto-promotable queue is currently empty; the remaining manual-decoder DLL now has a generated batch gate before runtime hooks.
