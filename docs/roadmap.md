...
6. System DLL coverage waves:
   ...
   - Wave 4: generated definitions for the remaining common Windows user-mode DLLs, with selected DbgHelp symbol-session hooks
...
1. Total APIs: 179.
2. `smoke_verified`: 112.
3. `hooked`: 4.
4. `definition_only`: 63.
...
1. Wave 4 now has stable metadata for `oleaut32.dll`, `secur32.dll`, `userenv.dll`, `dnsapi.dll`, `iphlpapi.dll`, `setupapi.dll`, `shlwapi.dll`, `wintrust.dll`, and `dbghelp.dll`; `SymInitializeW` and `SymCleanup` are already promoted to low-payload DbgHelp symbol-session hooks.
