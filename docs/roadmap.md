...
60. `generated/tier2-hook-plan.json` plans all 786 Tier 2 APIs with zero broad hooks enabled by default, including 279 API-set forwarder rows, 277 Windows-loader-resolved host rows, 507 missing-parameter return-only rows, and 41 risk-blocked rows.
61. `generated/tier2-profile-batch-plan.json` groups Tier 2 runtime-profile candidates into 104 review batches with zero broad hooks enabled by default: 16 resolved-host API-set batches, 87 missing-parameter return-only source-DLL batches, and 2 blocked unresolved API-set rows.
62. `generated/tier2-profile-review-manifest.json` ranks the Tier 2 profile batches with zero broad hooks enabled by default: 41 initial-candidate batches, 41 broader review-ready batches, 21 allowlist-gated batches, and 1 blocked batch.
63. The Tier 2 generic profile smoke verifies the generated generic runtime path for resolved-host API-set rows, return-only source-DLL rows, and resolver visibility for `GetProcAddress` and `LdrGetProcedureAddress`; the current smoke passes with 372 captured events.
64. `generated/tier3-hook-plan.json` plans all 1,359 Tier 3 APIs as design-review-only coverage with zero default-installable runtime hooks.
...
13. Stable generated metadata now covers `30,112` defined APIs and `30,092` runtime-monitorable generated/manual hook entries. The compact Tier 0 transport IDs still preserve the reviewed File I/O, loader, resolver, and Wave 2/3/4 low-payload smoke-verified API IDs, while the broader Tier 1/Tier 2 generic runtime path carries inventory API identity through generated metadata instead of assigning every API a compact transport ID.
14. Generated agent hook coverage currently reports `required=30092`, `manual=314`, `generated=29778`, `covered=30092`, and `chunks=59`.
...
18. `generated/tier1-hook-plan.json` is generated and validated for all `27,907` Tier 1 APIs, with `17,371` generic-ready rows, `10,536` ABI-blocked rows, and `4,435` risk-blocked rows.
19. `generated/tier2-hook-plan.json` is generated and validated for all `786` Tier 2 APIs, with `279` API-set rows, `277` resolved host DLL rows, `507` return-only rows, and `41` risk-blocked rows.
20. `generated/tier2-profile-batch-plan.json` is generated and validated from the Tier 2 hook plan. It groups the Tier 2 queue into `104` batches: `16` API-set host batches, `87` return-only module batches, and `2` blocked unresolved rows while keeping runtime hooks disabled by default.
21. `generated/tier2-profile-review-manifest.json` is generated and validated from the Tier 2 profile batch plan. It ranks the Tier 2 queue into `41` initial-candidate batches, `41` broader review-ready batches, `21` allowlist-gated batches, and `1` blocked batch while keeping runtime hooks disabled by default.
22. `generated/tier3-hook-plan.json` is generated and validated as a planning artifact for all `1,359` Tier 3 APIs, keeping every row `blocked_by_default` and requiring explicit allowlist, design review, smoke evidence, and overhead review before any future runtime hook promotion.
23. `generated/dll-batch-promotion-plan.json` is generated and validated as the committed-definition promotion gate. It currently groups `399` DLLs with `11,827` auto-promotable pointer/scalar APIs, `7,309` manual-decoder-required APIs, and `10,846` payload-policy-blocked APIs.
24. `generated/manual-decoder-batch-plan.json` is generated and validated from the DLL batch queue. It currently tracks `7,309` manual-decoder APIs across `243` DLLs plus `8,258` payload-blocked sibling rows.
...
6. Inventory tiers: 130 Tier 0 rows, 27,907 Tier 1 rows, 786 Tier 2 rows, and 1,359 Tier 3 rows.
7. Tier 2 hook plan: 279 API-set forwarders, 277 resolved host rows, 507 missing-parameter return-only rows, 41 risk-blocked rows, and zero broad hooks enabled by default.
8. Tier 2 profile batch plan: 104 review batches, with 16 resolved-host API-set batches, 87 missing-parameter return-only source-DLL batches, 2 blocked unresolved API-set rows, and zero broad hooks enabled by default.
9. Tier 2 profile review manifest: 41 initial-candidate batches, 41 broader review-ready batches, 21 allowlist-gated batches, 1 blocked batch, and zero broad hooks enabled by default.
10. DLL batch promotion plan: 399 DLLs, 11,827 auto-promotable APIs, 7,309 manual-decoder-required APIs, 10,846 payload-policy-blocked APIs, and zero broad hooks enabled by default.
11. Manual decoder batch plan: 7,309 manual APIs across 243 DLLs, 8,258 payload-blocked siblings, and zero broad hooks enabled by default.
