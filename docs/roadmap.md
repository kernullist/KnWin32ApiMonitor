...
14. The coverage report currently totals 52 `definition_only`, 4 `hooked`, and 123 `smoke_verified` APIs.
...
1. Total APIs: 179.
2. `smoke_verified`: 123.
3. `hooked`: 4.
4. `definition_only`: 52.
5. Microsoft-source inventory: 18,086 APIs, with 127 Tier 0 rows, 15,934 Tier 1 rows, 655 Tier 2 rows, and 1,370 Tier 3 rows.
...
7. DLL batch promotion plan: 25 DLLs, 127 already-live APIs, zero auto-promotable pointer/scalar APIs, 3 manual-decoder-required APIs, 49 payload-policy-blocked APIs, and zero broad hooks enabled by default.
...
6. Use the DLL batch promotion plan as the default promotion queue:
   ...
   - The auto-promotable queue is currently empty; manual-decoder DLLs must get a batch design before runtime hooks.
