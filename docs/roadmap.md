...
6. System DLL coverage waves:
   ...
   - Wave 4: generated definitions for the remaining common Windows user-mode DLLs, with selected OLEAUT32 lifecycle, USERENV environment-block destroy, DNSAPI record-list free, IPHLPAPI adapter/interface metadata, SETUPAPI device-info close, SHLWAPI path-exists query, and DbgHelp symbol-session hooks
...
20. `generated/manual-decoder-batch-plan.json` is generated and validated from the DLL batch queue. It covers `rpcrt4.dll!RpcMgmtEpEltInqDone` and `wintrust.dll!WTHelperProvDataFromStateData`, tracks same-DLL payload-blocked siblings, records allowed/forbidden evidence boundaries, and keeps both APIs blocked by default until same-DLL design review, x64/x86 smoke, payload absence, and overhead gates pass.
...
Status: refreshed on 2026-06-20 after Tier 2 coverage planning foundation, representative generic smoke coverage, and manual-decoder batch gate generation.

Current definition coverage:

1. Total APIs: 179.
2. `smoke_verified`: 124.
3. `hooked`: 4.
4. `definition_only`: 51.
5. Microsoft-source inventory: 18,086 APIs, with 128 Tier 0 rows, 15,933 Tier 1 rows, 655 Tier 2 rows, and 1,370 Tier 3 rows.
...
7. DLL batch promotion plan: 25 DLLs, 128 already-live APIs, zero auto-promotable pointer/scalar APIs, 2 manual-decoder-required APIs, 49 payload-policy-blocked APIs, and zero broad hooks enabled by default.
8. Manual decoder batch plan: 2 DLLs, 2 manual-decoder-required APIs, 4 same-DLL payload-blocked sibling APIs, and zero runtime hooks enabled by default.
...
3. Good first live candidates are low-volume lifecycle/query calls whose hooks can emit scalar handles, flags, return values, and pointer-only evidence without copying arbitrary payload buffers. The generated DLL batch plan currently has no auto-promotable runtime batches; future runtime expansion should start from the generated manual-decoder batch gates or a reviewed payload-sensitive family plan.
