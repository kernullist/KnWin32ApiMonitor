...
8. Zstd-compressed `.knapm` chunks, metadata database indexing, and long-running daemon-owned replay catalogs.
...
3. `validate-session --session <path.knapm>` verifies manifest/index identity, chunk hashes, event counts, batch contiguity, record monotonicity, finalized vs partial writer state, and final cleanup evidence without launching or mutating the target.
4. `replay-session --session <path.knapm>` validates first, then replays indexed trace chunks into the existing `session-replay` result shape without injecting or launching a process.
5. Legacy `manifest.json` + JSONL session directories remain supported by the same validate/replay commands.
