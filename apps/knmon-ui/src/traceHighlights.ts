...
function highSignalApiReason(event: TraceEvent): string | null {
  // Compare case-insensitively so the highlighted-row count matches the
  // case-insensitive query clauses generated by coverageHintClauses().
  const apiLower = event.api.trim().toLowerCase();
  const moduleLower = event.module.trim().toLowerCase();

  if (apiLower.startsWith("nt") || moduleLower === "ntdll.dll") {
    return "Native NT API boundary";
  }

  if (apiLower.startsWith("reg")) {
    return "Registry API family";
  }

  if (apiLower === "openprocesstoken" || apiLower === "lookupprivilegevaluew") {
    return "Security token or privilege API family";
  }

  if (apiLower.startsWith("bcrypt")) {
    return "CNG crypto provider/RNG API family";
  }

  if (apiLower.startsWith("crypt") || apiLower.startsWith("cert")) {
    return "Certificate or crypto message API family";
  }

  if (apiLower.startsWith("winhttp") || apiLower.startsWith("internet")) {
    return "HTTP client session API family";
  }

  if (apiLower.startsWith("rpc")) {
