...
6. Writes bounded binary API records for `CreateFileW`, `CreateFileA`, `NtCreateFile`, `ReadFile`, `WriteFile`, `CloseHandle`, loader/resolver events, and the first selected Winsock API slice into the required shared-memory transport when `KNMON_TRANSPORT_NAME` is supplied.
