...
def direct_cli_control():
    fixture = Path(tempfile.mkdtemp(prefix="inventory-clean-cli-", dir=inventory.ROOT / "build"))
    code = fixture / "tools/security"
    code.mkdir(parents=True)
    for name in ("dependency_inventory.py", "tauri_backport.py"):
        shutil.copyfile(inventory.ROOT / "tools/security" / name, code / name)
    before = {file.name: inventory.digest(file.read_bytes()) for file in code.iterdir()}
    environment = {key: value for key, value in os.environ.items()
                   if key.upper() not in ("PYTHONDONTWRITEBYTECODE", "PYTHONPYCACHEPREFIX")}
    command = [sys.executable, "-X", "utf8", str(code / "dependency_inventory.py"), "--help"]
    result = subprocess.run(command, cwd=fixture, env=environment, capture_output=True, timeout=30)
    (fixture / "command.json").write_bytes(inventory.encoded({"command": command, "cwd": str(fixture)}))
    (fixture / "stdout.log").write_bytes(result.stdout)
    (fixture / "stderr.log").write_bytes(result.stderr)
    inventory.require(result.returncode == 0 and b"usage:" in result.stdout, "Direct inventory CLI did not execute.")
    inventory.require(all(file.is_file() for file in code.iterdir()) and
                      {file.name: inventory.digest(file.read_bytes()) for file in code.iterdir()} == before,
                      "Direct inventory CLI changed source files or generated bytecode.")
    print("Passed: direct CLI leaves copied sources unchanged without a bytecode environment override: " + str(fixture))
...
def main():
    ...
    renamed = copy.deepcopy(npm)
    renamed["metadata"]["component"]["name"] = "different-checkout-directory"
    next(row for row in renamed["components"] if row["bom-ref"].startswith("@knmon/"))["name"] = "different-workspace-directory"
    inventory.require(inventory.npm_components(renamed, lock) == inventory.npm_components(npm, lock),
                      "npm directory labels changed the canonical inventory.")
    print("Passed: root and scoped workspace directory labels cannot change canonical package names.")
    for label in ("duplicate npm identity", "missing npm component", "wrong npm integrity", "removed npm edge",
                  "wrong npm registry name", "wrong npm root purl", "wrong npm scoped purl", "wrong npm root"):
        ...
        elif label == "wrong npm registry name":
            next(row for row in value["components"] if row.get("hashes"))["name"] = "wrong-package"
        elif label == "wrong npm root purl":
            value["metadata"]["component"]["purl"] = "pkg:npm/wrong-package@0.1.0"
        elif label == "wrong npm scoped purl":
            next(row for row in value["components"] if row["bom-ref"].startswith("@knmon/"))["purl"] = "pkg:npm/knmon-ui@0.1.0"
        elif label == "wrong npm root":
            value["metadata"]["component"], value["components"][0] = value["components"][0], value["metadata"]["component"]
        else:
    ...
    with tempfile.TemporaryDirectory(prefix="inventory-negatives-", dir=inventory.ROOT / "build") as temporary:
        ...
        for label in ("artifact hash", "stale source", "missing target", "path escape", "missing graph edge", "wrong feature scope",
                      "forged backport pedigree", "forged npm canonical name"):
            ...
            elif label == "forged npm canonical name":
                bom = inventory.read_json(case / "bom.cdx.json")
                component = next(row for row in bom["components"] if row["bom-ref"] == "npm:" + npm["metadata"]["component"]["bom-ref"])
                component["name"] = "wrong-checkout-label"
                (case / "bom.cdx.json").write_bytes(inventory.encoded(bom))
                evidence["artifacts"]["bom.cdx.json"] = inventory.digest((case / "bom.cdx.json").read_bytes())
            else:
                modified = copy.deepcopy(current)
    ...
    print("Dependency evidence adversarial validation PASS: 28 negative controls and 2 positive groups.")
