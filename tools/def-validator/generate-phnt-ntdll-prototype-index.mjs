...
const phntDir = path.resolve(argumentValue("--phnt-dir", process.env.PHNT_DIR ?? ""));
const outputPath = path.join(repoRoot, "generated", "phnt-ntdll-prototype-index.json");
const exportBaselinePath = path.join(repoRoot, "generated", "ntdll-win11-export-baseline.json");

if (!phntDir || phntDir === path.parse(phntDir).root || !fs.existsSync(phntDir))
{
  throw new Error("Provide a PHNT checkout with --phnt-dir=<path> or PHNT_DIR.");
}
...
const sourceCommit = gitOutput(phntDir, ["rev-parse", "HEAD"]);
const sourceRemote = gitOutput(phntDir, ["config", "--get", "remote.origin.url"]);
const prototypes = buildPrototypeIndex(phntDir, exportNames);

const document = {
  schemaVersion: "0.1.0",
  source: {
    name: "PHNT",
    repository: sourceRemote || "https://github.com/winsiderss/phnt",
    commit: sourceCommit,
    note: "Generated from PHNT Native API headers for ntdll export prototype enrichment."
  },
  baseline: {
...
if (write)
{
  fs.writeFileSync(outputPath, `${stableStringify(document)}\n`, "utf8");
}

console.log(`Generated PHNT ntdll prototype index. exports=${exportNames.size} prototypes=${prototypes.length} commit=${sourceCommit}`);
...
function gitOutput(cwd, gitArgs)
{
  ...
  try
  {
    output = childProcess.execFileSync("git", gitArgs, {
      cwd,
      encoding: "utf8",
...
function buildPrototypeIndex(root, exportNames)
{
  ...
  for (const fileName of fs.readdirSync(root).filter((name) => name.endsWith(".h")).sort())
  {
    const filePath = path.join(root, fileName);
    const text = fs.readFileSync(filePath, "utf8");
    for (const statement of prototypeStatements(text))
    {
      const parsed = parsePrototypeStatement(statement, fileName);
...
{
  const clean = stripComments(text).replace(/^\s*#.*$/gm, " ");
  const statements = [];
  let current = "";
  let braceDepth = 0;
  ...
  for (let index = 0; index < clean.length; ++index)
  {
    ...
    if (ch === ";" && braceDepth === 0)
    {
      ...
      if (
        /\b(?:NTSYSAPI|NTSYSCALLAPI|WINBASEAPI|DECLSPEC_IMPORT)\b/.test(statement) &&
        statement.includes("(") &&
...
function parsePrototypeStatement(statement, fileName)
{
  ...
  do
  {
    ...
    const returnText = normalizeReturnType(before.slice(0, nameMatch.index));
    const parameters = parseParameters(argsText);
    result = {
      name,
      returnType: returnText || "ULONG_PTR",
...
function normalizeReturnType(text)
{
  return stripPrototypeNoise(text)
    .replace(/\b(?:NTSYSAPI|NTSYSCALLAPI|WINBASEAPI|DECLSPEC_IMPORT|NTAPI|WINAPI|APIENTRY|STDAPIVCALLTYPE|STDAPI|CDECL|__cdecl|DECLSPEC_NORETURN|NORETURN|FORCEINLINE|EXTERN_C|CONST)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
{
  if (/\bSTDAPIVCALLTYPE\b|\b__cdecl\b|\bCDECL\b/.test(text))
  {
    return "cdecl";
  }
