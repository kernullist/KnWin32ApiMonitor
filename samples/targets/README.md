...
It builds as `knmon-sample-fileio.exe` through the native CMake project and performs deterministic File I/O plus small local registry, RPCRT4, and Winsock probes:
...
10. Call `WSAStartup`, `getaddrinfo("localhost", "80", ...)`, `socket`, `WSAGetLastError`, `closesocket`, `freeaddrinfo`, and `WSACleanup`.
11. Attempt missing wide-char and ANSI paths for error coverage.

The native helper uses this executable for controlled launch-time early-bird APC agent loading and bounded File I/O, loader, resolver, selected registry, selected RPCRT4 binding, and selected Winsock hook capture. Arbitrary already-running process injection remains out of scope.
