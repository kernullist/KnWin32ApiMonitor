...
It builds as `knmon-sample-fileio.exe` through the native CMake project and performs deterministic File I/O plus small local registry, RPCRT4, bcrypt CNG, crypt32 certificate/message-handle, WinHTTP session, WinINet session, and Winsock probes:
...
13. Open and close a `wininet.dll` session with user agent `KNMonWinInetSample/1.0`, direct access, and no network request.
14. Call `WSAStartup`, `getaddrinfo("localhost", "80", ...)`, `socket`, `WSAGetLastError`, `closesocket`, `freeaddrinfo`, and `WSACleanup`.
15. Attempt missing wide-char and ANSI paths for error coverage.
